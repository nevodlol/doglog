      ...
      <Row gutter={24} style={{ marginTop: 10 }}>
        <Col span={10}>
          <Dragger {...uploadProps} style={{ height: "100%", minHeight: 280 }}>
            {fileList.length === 0 ? (
              <>
                ...
                <p style={{ fontSize: 12, color: "rgba(0,0,0,0.45)" }}>изображения до 10 Мб</p>
              </>
            ) : (
          ...
          {fileList.length > 0 && (
            <div
              onClick={() => setFileList([])}
              style={{ textAlign: "center", marginTop: 8, color: "#ff4d4f", cursor: "pointer", fontSize: 12 }}
            >
        ...
        <Col span={14}>
          <Form form={form} layout="vertical">
            <Form.Item label="Кличка" name="name" rules={[{ required: true, message: "Введите кличку" }]}>
              <Input showCount maxLength={20} />
            </Form.Item>

            <Form.Item label="Пол" name="gender" rules={[{ required: true, message: "Выберите пол" }]}>
              <Select options={[{ value: "male", label: "кобель" }, { value: "female", label: "сука" }]} />
            </Form.Item>

            <Form.Item label="Дата рождения" name="birthdate" rules={[{ required: true, message: "Укажите дату рождения" }]}>
              <DatePicker style={{ width: 150 }} format="DD.MM.YYYY" placeholder="" />
            </Form.Item>

            <Form.Item label="Порода" name="breed" rules={[{ required: true, message: "Введите породу" }]}>
              <Input />
            </Form.Item>

            <Form.Item label="Окрас" name="color" rules={[{ required: true, message: "Введите окрас" }]}>
              <Input />
            </Form.Item>

            <Form.Item label="Клеймо (чип)" name="chip">
              <Input placeholder="Если не чипирована, оставьте это поле пустым" />
            </Form.Item>
          </Form>
        </Col>
      </Row>
